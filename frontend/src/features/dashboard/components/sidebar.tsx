...
const sections = [
  {
    title: 'Général',
    links: [{ name: 'Accueil', href: '/dashboard', icon: Home }],
  },
  {
    title: 'Contenu',
    links: [
      {
        ...
        icon: FileText,
        badge: '8',
        badgeColor: 'bg-[#7B51FA]/10 text-[#7B51FA]',
      },
  ...
  return (
    <>
      ...
      >
        {/* Brand & Logo */}
        <div
          className={`flex h-16 items-center pt-4 mb-2 transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-6'}`}
        >
          ...
          >
            ...
            >
              <div className="flex flex-col items-start translate-y-[1px]">
                <span className="font-bold text-slate-800 font-outfit text-[15px] leading-none text-left">
                  Team Dev
                </span>
                <span className="text-[10px] font-bold text-[#7B51FA] mt-1 tracking-wider uppercase">
                  Workspace
                </span>
              </div>
              ...
              {isWorkspaceOpen && (
                ...
                >
                  ...
                  <div className="flex items-center justify-between rounded-lg bg-gray-50/80 px-3 py-2.5 cursor-pointer hover:bg-gray-100 transition-colors">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded bg-gradient-to-br from-[#7B51FA] to-[#6A41E8] flex items-center justify-center text-white text-xs font-bold">
                        TD
                      </div>
        ...
        <div
          className={`flex-1 overflow-y-auto scrollbar-hide py-2 transition-all duration-300 ${isCollapsed ? 'px-2 space-y-2' : 'px-4 space-y-3'}`}
        >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`flex items-center justify-center w-9 h-9 rounded-lg transition-colors ${isActive ? 'bg-white shadow-sm' : 'group-hover:bg-white group-hover:shadow-sm'}`}
                        >
                          <Icon
                            className={`h-[20px] w-[20px] ${isActive ? 'text-[#7B51FA]' : 'text-slate-400 group-hover:text-slate-600'}`}
                          />
                        </div>
                        {!isCollapsed && (
                          <motion.span
                      ...
                      {!isCollapsed && link.badge && (
                        <span
                          className={`flex h-5 items-center justify-center rounded-full px-2 min-w-[20px] text-[10px] font-black shadow-sm ${link.badgeColor || 'bg-gray-300 text-white'}`}
                        >
                          {link.badge}
                        </span>
                      )}
        ...
        <div
          className={`p-4 bg-white border-t border-gray-50 z-10 transition-all duration-300 ${isCollapsed ? 'flex flex-col items-center gap-4' : ''}`}
        >
          {isCollapsed ? (
            <>
              <div className="relative group cursor-pointer">
                <div className="h-10 w-10 overflow-hidden rounded-full border-2 border-[#7B51FA]/10 bg-gray-50">
                  <Image
                    src="https://api.dicebear.com/7.x/avataaars/svg?seed=Martin"
                    alt="User"
                ...
                <div className="flex items-center gap-3">
                  <div className="relative">
                    ...
                    <div className="absolute -bottom-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-white shadow-sm border border-gray-100">
                      <div className="flex h-3 w-3 items-center justify-center rounded-full bg-[#7B51FA] text-white text-[8px] font-bold">
                        Pf
                      </div>
                    </div>
                  </div>
